...
export default function ExploreRooms() {
  ...
  useEffect(() => {
    ...
    // لو في توكن
    if (storedToken) {
      fetchRooms(storedToken, 1, start || undefined, end || undefined);
    }
  }, [start, end]);
            ...
            <div className="flex justify-center mt-12">
              <div className="flex space-x-1">
                {currentPage > 1 && (
                  <button
                    onClick={() =>
                      fetchRooms(token, currentPage - 1, start!, end!)
                    }
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                  (page) => (
                    <button
                      key={page}
                      onClick={() => fetchRooms(token, page, start!, end!)}
                      className={`min-w-9 rounded-full py-2 px-3.5 text-sm ${
                        currentPage === page
